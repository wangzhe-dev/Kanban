...
-->
<template>
  <div class="container-StraightSection">
    <div class="t-profile-screen" ref="appRef">
      <ZoomPageSwitch v-model="drillVisible" :origin="drillOrigin">
        <template #main>
          <MainLayout activeTab="StraightSection" @tab-click="handleTabClick">
            <template #left>
              <Card title="工单信息" :showDrill="true"
                @drill-click="(p) => openDrill('workorder', p)" :bodyPadding="'20px'" :showTimeRange="false"
                column="left" position="single" :totalCards="1">
            ...
            <template #center-top>
              <Card title="生产任务情况" :flex="3" column="center" position="top" :totalCards="3" :showTimeRange="true"
            ...
            <template #center-middle>
              <Card title="报工情况" :showDrill="true" @drill-click="(p) => openDrill('report', p)" :flex="3"
                column="center" position="middle" :totalCards="3" :bodyPadding="'40px 20px 20px 20px'">
                <ReportDetails :reportData="reportDetailsData" />
              </Card>
            </template>

            <template #center-bottom>
              <Card title="缓存情况" :showDrill="true" @drill-click="(p) => openDrill('cache', p)" :flex="3" column="center"
                position="bottom" :totalCards="3" :bodyPadding="'50px'">
            ...
            <template #right-top>
              <Card title="当日生产" :showDrill="true" @drill-click="(p) => openDrill('daily', p)"
                :titleMargin="'20px 0 0 0'" :flex="3" column="right" position="top" :totalCards="3"
            ...
            <template #right-middle>
              <Card title="产量统计" :titleMargin="'20px 0 0 0'" :timeTabsMargin="'10px 0 0 0'" :flex="3" column="right"
                ...
                :bodyPadding="'60px 20px 20px 20px'"
                @time-range-change="(tab) => handleBodyTabChange('production', tab)">
                <ProductionStats :columns="currentProductionStats.columns" :tableData="currentProductionStats.data" />

              </Card>
            </template>
            <template #right-bottom>
              <Card title="报工情况" :showDrill="true" @drill-click="(p) => openDrill('report', p)"
                :titleMargin="'20px 0 0 0'" :flex="3" column="right" position="bottom" :totalCards="3"
                :showTimeRange="false" :bodyPadding="'60px 20px 20px 20px'">
        ...
        <template #detail>
          <component :is="activeDrillComponent" @back="handleDrillBack" />
        </template>
      </ZoomPageSwitch>
    </div>
  </div>
...
import ZoomPageSwitch from "../components/ZoomPageSwitch.vue";
import CacheDetailTablePage from "./comp/CacheDetailTablePage.vue";
import DailyProductionTablePage from "./comp/DailyProductionTablePage.vue";
import ReportTablePage from "./comp/ReportTablePage.vue";
import WorkOrderTablePage from "./comp/WorkOrderTablePage.vue";
...
export default {
  ...
  components: {
    ...
    WorkOrderTablePage,
    ReportTablePage,
    CacheDetailTablePage,
    DailyProductionTablePage,
    DailyProduction,
    OrderStatistics,
    PlanCompletion,
    ProductionStats,
  ...
  computed: {
    activeDrillComponent() {
      const map = {
        workorder: "WorkOrderTablePage",
        report: "ReportTablePage",
        cache: "CacheDetailTablePage",
        daily: "DailyProductionTablePage",
      };
      return map[this.drillKey] || "WorkOrderTablePage";
    },
    reportDetailsData() {
  ...
  methods: {
    ...
    openDrill(key, payload) {
      console.log("Drill clicked:", key, payload);
      this.drillKey = key;
      this.drillOrigin = payload && payload.origin ? payload.origin : null;
